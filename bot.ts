...
function getNextQuarterHours(count: number = 12): { label: string; value: string }[] {
    const now = new Date();
    ...
    for (let i = 0; i < count; i++) {
        ...
        const timeStr = slot.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
        options.push({
            label: timeStr,
...
client.on(Events.InteractionCreate, async (interaction: Interaction) => {
    ...
    } else if (interaction.isStringSelectMenu() && interaction.customId === 'time_select') {
